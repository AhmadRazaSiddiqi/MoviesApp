...
export default function ActorMovies() {
  ...
  const { actorData: dataString } = useLocalSearchParams();
  const data = dataString ? JSON.parse(dataString as string) : null;
  useEffect(() => {
    if (data?.id) {
      axios
        .get(
          `https://api.themoviedb.org/3/person/${data.id}/movie_credits?api_key=${API_KEY}`
        )
  ...
  return (
    <View className="flex-1 bg-neutral-800 p-4">
      <Text className="text-white text-3xl font-bold mb-4">
        {data?.name ? `${data.name}'s Movies` : "Actor's Movies"}
      </Text>
      <FlatList
        data={movies}
        numColumns={2}
